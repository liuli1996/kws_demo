...
def train(config):
    ...
    schedule_steps = config["schedule"] # np.inf
    ...
    for epoch_idx in range(config["n_epochs"]): # 90 epochs
        for batch_idx, (model_in, labels) in enumerate(train_loader):
            ...
            # lr和schedule_steps是一个list，表示经过schedule_steps，lr下降一次
        ...
        if epoch_idx % config["dev_every"] == 0:  # 每10个epoch evaluate一次
            model.eval()
            accs = []
...
def main():
    output_file = os.path.join(os.path.dirname(os.path.realpath(__file__)), "..", "model", "conv_2layer_fc_2layer_model_lmcl.pt")
    parser = argparse.ArgumentParser()
    parser.add_argument("--model", choices=[x.value for x in list(mod.ConfigType)], default="customized-model", type=str)
    config, _ = parser.parse_known_args()

    global_config = dict(no_cuda=False, n_epochs=90, lr=[0.01], schedule=[np.inf], batch_size=100, dev_every=1, seed=0,
        use_nesterov=False, input_file=None, output_file=output_file, gpu_no=0, cache_size=32768, momentum=0.9, weight_decay=0.00001)
